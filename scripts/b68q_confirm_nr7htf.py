#!/usr/bin/env python3
"""b68 STRATEGY LAB ROUND 17 CONFIRM — nr7 x H4-trend on cached + W1..W4.

Five legs (b76 discipline: every leg records its own span and the overlap
with the cached set; W1..W4 are the four independent windows b74 spent in
rounds 11-14, zero overlap asserted in the windows ledger). The funnel is
re-measured on the SAME bars as every arm, and its rows must reproduce the
shipped b68p ledger exactly (same bars, same harness — drift is a bug,
pinned by tests/test_b68q_nr7htf_lab.py).

Arms per leg: CURRENT_FUNNEL (live replay), nr7_w10_control (round 5, the
ungated ingredient — b72 rule 3), nr7_h4t_agree / nr7_h4t_disagree (the
gated arm and what the gate DROPS — b74's selection-ordering check), and
lane_funnel_then_nr7htf (b70 capacity: funnel first, gated arm only on free
bars). Every arm row carries its BUY/SELL trade mix (b78).
...
Decision rules shipped with the ledger:
  b74 all-windows: an arm is REPLICATED only if its ladder_ts exp_R beats
      ...
      ties never beat);
  b77 pre-flight: margins sorted by TIME (never by label);
  selection ordering (the round-9 killer check): agree > disagree on the
...
OUT = os.path.join(_ROOT, "data", "backtest", "b68q_nr7htf_confirm.json")
WINDOWS = ("W1", "W2", "W3", "W4")
LEGS = ("cached",) + WINDOWS
ARMS = ("CURRENT_FUNNEL", "nr7_w10_control", "nr7_h4t_agree",
        "nr7_h4t_disagree", "lane_funnel_then_nr7htf")

# round-16 shipped ledger: the funnel rows on W1..W4 must match it exactly
CONTINUITY = os.path.join(_ROOT, "data", "backtest", "b68p_runway_confirm.json")


def funnel_signals(m15, h1, h4):
    """The live funnel replayed bar-by-bar on one dataset — the exact
    convention b68n/b68o/b68p's measure_leg uses (only fully-closed HTF
    context bars, 80-bar H1/H4 windows, 120-bar M15 window)."""
...
def measure_leg(name, m15, h1, h4):
    """One leg: funnel + the whole round-17 arm set through the b71 harness,
    ...
    def lane(row):
        return funnel(row) or wrap(
            lambda i: q.gated(i, prep, "agree"))(row)

    arms = [("CURRENT_FUNNEL", funnel),
            ("nr7_w10_control", wrap(lambda i: nf.nr7_break(i, wide=True))),
            ("nr7_h4t_agree", wrap(lambda i: q.gated(i, prep, "agree"))),
            ("nr7_h4t_disagree", wrap(lambda i: q.gated(i, prep, "dis"))),
            ("lane_funnel_then_nr7htf", lane)]
    ...
    out = {"_time_stop_bars": lh.live_time_stop_bars(m15),
           ...
           "_probe": q.gate_probe(prep),
           "_stretch_probe": q.stretch_probe(prep)}
...
def verdict(led, windows=WINDOWS):
    ...
    cell carries the arm's BUY/SELL mix. Selection ordering (agree vs the
    dropped disagree) is counted per window."""
    v = {}
    ...
    v["replicated_in_all"] = {
        ...
        for arm in ("nr7_h4t_agree",)}
    wins_ok = sum(1 for w in windows
                  if v[w]["nr7_h4t_agree"]["exp_R"] is not None
                  and v[w]["nr7_h4t_disagree"]["exp_R"] is not None
                  and v[w]["nr7_h4t_agree"]["exp_R"]
                  > v[w]["nr7_h4t_disagree"]["exp_R"])
    v["selection_ordering"] = {"agree_gt_cut_windows": wins_ok,
                               "of": len(windows)}
    return v
...
def main():
    ...
    c = json.load(open(q.CACHED_PATH))
    led = {"_note": "round 17: NR7 breakout x H4-trend gate measured on "
    ...
    led["_b77_preflight"] = ld.preflight(
        led, ["nr7_h4t_agree", "nr7_h4t_disagree"], list(WINDOWS), meta)
