#!/usr/bin/env python3
"""b68f-confirm — nr7_break vs the CURRENT live funnel on FRESH data.
...
nr7_break_c +0.584R (n=326), nr7_break_w10 +0.598R (n=218) vs funnel
+0.854R — the strongest cached ladder numbers since round 4's pdh arms,
and by a wide margin over rounds 1-3 (0.19-0.44 band). Per the b68
round-1 METHOD RULE the funnel is RE-MEASURED on the SAME fresh bars.

The lab module closes over the module-level M15/IDX, so the confirm
repoints BOTH to the fresh bars before running the arms (same pattern as
b68e_confirm_pdh).
"""
import sys, os, json
...
from scripts import b68f_nr7_lab as lab
from scripts.b68f_nr7_lab import nr7_break
...
def main():
    ...
    def lane(row):
        """Additive-lane probe (round-4 machinery): funnel first, arm only on
        bars the funnel leaves empty, one position at a time. NOT a merit-bar
        test — it feeds b70's capacity question with a SECOND lane candidate
        (nr7 fires ~2x more often than pdh on fresh data)."""
        return funnel(row) or wrap(lambda i: nr7_break(i, wide=True))(row)

    arms = [("CURRENT_FUNNEL", funnel),
            ("nr7_break_c", wrap(lambda i: nr7_break(i, wide=False))),
            ("nr7_break_w10", wrap(lambda i: nr7_break(i, wide=True))),
            ("lane_funnel_then_nr7w10", lane)]
    out = {}
    ...
    p = os.path.join(_ROOT, "data", "backtest", "b68f_nr7_confirm.json")
