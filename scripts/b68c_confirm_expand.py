#!/usr/bin/env python3
"""b68c CONFIRM — atr_expand vs the CURRENT live funnel on FRESH data.
...
atr_expand_all ladder +0.362R (n=104), atr_expand_lny +0.344R (n=69) vs
funnel +0.854R -> already short on cached. Per the b68 round-1 METHOD RULE
the funnel is RE-MEASURED on the SAME fresh bars (0.854 is cached-set
specific; on fresh 6000 bars the funnel itself scored 0.576 in round 1), so
this settles the arm honestly on both numbers.
"""
import sys, os, json
...
from scripts import b68_expand_lab as lab
from scripts.b68_expand_lab import expansion_continuation, LONDON_NY
...
def main():
    ...
    # expansion_continuation closes over the MODULE-level M15/IDX — repoint
    lab.M15 = m15
    lab.IDX = idx_of
    ...
    arms = [("CURRENT_FUNNEL", funnel),
            ("atr_expand_all", wrap(lambda i: expansion_continuation(i))),
            ("atr_expand_lny", wrap(lambda i: expansion_continuation(i, sessions=LONDON_NY)))]
    out = {}
    ...
    p = os.path.join(_ROOT, "data", "backtest", "b68c_expand_confirm.json")
