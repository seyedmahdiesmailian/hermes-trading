#!/usr/bin/env python3
"""b68e-confirm — pdh_break vs the CURRENT live funnel on FRESH data.

b68 merit bar: an arm must beat the funnel on BOTH the cached 3000-bar set
AND one fresh fetch, same engine, same live b60 ladder. Cached ladder:
pdh_break_t50 +0.493R (n=67), pdh_break_w10 +0.627R (n=54) vs funnel
+0.854R — short on cached, but the strongest ladder numbers this loop has
produced (previous rounds topped out at 0.44 fresh). Per the b68 round-1
METHOD RULE the funnel is RE-MEASURED on the SAME fresh bars so the arm
settles honestly on both numbers.

The lab module's LEVELS table is built from the CACHED M15 at import time,
so the confirm rebuilds it from the fresh bars (same trading-day rule)
before running the arm — otherwise it would trade against stale levels.
"""
import sys, os, json
...
from scripts import b68e_pdh_lab as lab
from scripts.b68e_pdh_lab import pdh_break
...
def main():
    ...
    # pdh_break closes over the MODULE-level M15/LEVELS — repoint BOTH to the
    # fresh data (LEVELS is built at import from the cached file).
    lab.M15 = m15
    lab.IDX = idx_of
    buckets = {}
    for r in m15:
    ...
    lab.LEVELS = levels
    print("fresh trading days with levels:", len(levels), flush=True)
    ...
    def lane(row):
        """Additive-lane probe: take the funnel when it fires, otherwise the
        arm. This is NOT a merit-bar test (the bar is standalone exp_R on both
        sets) — it measures whether the arm's edge is COMPLEMENTARY, i.e.
        whether it trades bars the funnel leaves empty. A lane that raises
        total R while diluting per-trade R is a different decision (capacity)
        from an arm that replaces the funnel, and needs its own gate."""
        return funnel(row) or wrap(lambda i: pdh_break(i, 1.0))(row)

    arms = [("CURRENT_FUNNEL", funnel),
            ("pdh_break_t50", wrap(lambda i: pdh_break(i, 0.5))),
            ("pdh_break_w10", wrap(lambda i: pdh_break(i, 1.0))),
            ("lane_funnel_then_w10", lane)]
    out = {}
    ...
    p = os.path.join(_ROOT, "data", "backtest", "b68e_pdh_confirm.json")
