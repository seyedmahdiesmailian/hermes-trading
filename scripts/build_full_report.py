...
OUT = '/home/ai/hermes-trading/reports/Hermes_System_Report_Full_1405-06-13.docx'
CH = '/tmp'
...
callout(doc, 'نکتهٔ صحت‌سنجی (مهم‌ترین تغییر این نسخه)',
        ...
        'اساس position_id، ارقام صحیح به‌دست آمد: ۲۳ پوزیشن، سود ناخالص '
        '۱۸۶.۱۰ دلار، کارمزد ۹.۹۰- دلار، و سود خالص ۱۷۶.۲۰ دلار. '
...
para(doc,
     'در ۱۳ روز اخیر ۲۳ پوزیشن توسط موتور داخلی باز و بسته شده است: '
     '۱۷ برد و ۶ باخت، سود خالص ۱۷۶.۲۰ دلار پس از کسر کارمزد، و افت '
     'سرمایهٔ حداکثری ۱۶۷.۱۳- دلار. این سود عمدتاً از یک برد بزرگ '
     '(۸۴+ دلار) و چند برد متوسط آمده، نه از تداوم سود.')
...
callout(doc, 'مسیر سیگنال تلگرام تاکنون صفر ترید اجرا کرده است',
        ...
        'نرسیده (executed=True = صفر). همهٔ ۲۳ پوزیشن واقعی از موتور تحلیل '
...
make_table(doc, ['#', 'تصمیم', 'توصیهٔ سیستم', 'اولویت'], [
    ...
    ['D-3', 'آیا حساب از دمو به واقعی منتقل شود؟',
     'خیر — ۲۳ پوزیشن برای نتیجه‌گیری کافی نیست', 'متوسط'],
    ['D-4', 'آیا پروفایل ریسک (میانگین باخت ۵۸- در برابر برد ۳۱+) اصلاح شود؟',
...
kpi_grid(doc, [
    ...
    ('725', 'تست سبز', 'g'),
    ('0', 'پوزیشن باز', 'g'),
    ('$176.20', 'سود خالص ۱۳ روز', 'g'),
    ('73.9%', 'نرخ برد', 'g'),
    ('-$167.13', 'حداکثر افت سرمایه', 'r'),
    ('0', 'ترید از سیگنال', 'a'),
], per_row=4)
page_break(doc)
...
make_table(doc, ['گره', 'IP', 'نقش', 'اجزا'], [
    ['Ubuntu 24.04', '192.168.10.18', 'مغز — تحلیل، تصمیم، ریسک، زمان‌بندی',
     '۶ سرویس systemd · ۷ کران · ۴۰۳ فایل پایتون'],
    ['Windows Server VM', '192.168.10.51', 'بازوی اجرا — فقط MT5',
...
para(doc, 'موتوری که تمام ۲۳ ترید واقعی از آن آمده است. برخلاف مسیر سیگنال، '
...
make_table(doc, ['کانال', 'شناسه', 'نقش'], [
    ['RADIN', '-1003417665507', 'کانال اصلی — ۶۶۷ پا در رپلی'],
...
# ═══════════════════════ ۸ PERFORMANCE ═════════════════════════════════
heading(doc, '۸. عملکرد — ۲۳ پوزیشن در ۱۳ روز', 1)
...
make_table(doc, ['شاخص', 'مقدار', 'تفسیر'], [
    ['پوزیشن باز شده', '23', 'همه بسته — صفر باز'],
    ['بردها / باخت‌ها', '17 / 6', 'نرخ برد ۷۳.۹٪'],
    ['سود ناخالص', '+$186.10', 'پیش از کارمزد'],
    ['کارمزد', '-$9.90', 'swap صفر'],
    ['سود خالص', '+$176.20', 'معیار واقعی'],
    ['میانگین برد', '+$31.43', '—'],
    ['میانگین باخت', '-$58.04', '۱.۸۵ برابر بزرگ‌تر از برد'],
    ['بهترین / بدترین', '+$84.24 / -$105.15', 'یک ترید ۴۸٪ سود کل'],
    ['حداکثر افت سرمایه', '-$167.13', 'تجمعی، فقط پوزیشن بسته'],
    ['لات مصرفی', '0.01 تا 0.16', 'میانگین 0.072 — متغیر'],
    ['سود نرمال‌شده @0.01', '+$44.21', 'برابر‌سازی حجم'],
    ['روزهای فعال', '6 از 13', 'بازار بسته یا بدون ستاپ'],
], widths=[42, 34, 82], center_cols=(1,))

picture(doc, f'{CH}/chart_equity.png',
        caption='شکل ۱ — منحنی سود تجمعی ۲۳ پوزیشن. سود عمدتاً از یک '
                'پلهٔ بزرگ می‌آید، نه شیب پیوسته.')
picture(doc, f'{CH}/chart_pertrade.png',
        caption='شکل ۲ — نتیجهٔ تک‌تک پوزیشن‌ها. تعداد برد زیاد است اما '
                'عمق باخت‌ها بیشتر از ارتفاع بردهاست.')

heading(doc, '۸.۲ تفسیر صادقانه', 2)
callout(doc, 'پروفایل شکننده: سود از اجتناب از ضرر بزرگ می‌آید، نه برد بزرگ',
        'نرخ برد ۷۳.۹٪ خوب است، اما ترکیب آن با میانگین باخت ۵۸- در برابر '
        'میانگین برد ۳۱+ یعنی سیستم با یک باخت بد، سه برد را پاک می‌کند. '
        'بدون آن برد ۸۴+ دلاری، سود خالص به حدود ۹۲ دلار می‌رسید. '
...
picture(doc, f'{CH}/chart_channels.png',
        caption='شکل ۳ — مقایسهٔ ۷ کانال: سود خالص و حداکثر افت سرمایه '
                'در رپلی ۹۰ روزه با لات ثابت.')

heading(doc, '۹.۱ جدول نهایی', 2)
make_table(doc, ['کانال', 'پا', 'سود A', 'سود B', 'وین‌ریت', 'MaxDD', 'حکم'], [
    ['goldfree', '349', '+$948', '+$432', '82%', '-$59', 'تنها کانالی که می‌ارزد'],
    ['radin', '667', '+$595', '+$192', '53%', '-$486', 'بی‌کیفیت — افت سنگین'],
    ['gtmo', '63', '+$106', '+$94', '68%', '-$14', 'کوچک ولی تمیز'],
    ['goldsystem', '37', '+$23', '+$26', '57%', '-$22', 'حاشیهٔ ناچیز'],
    ['otsfx', '6', '+$19', '-$23', '33%', '-$36', 'اصلاً طلا نیست'],
    ['gtmofx', '115', '-$84', '-$135', '76%', '-$102', 'ضررده'],
    ['olivex', '47', '-$244', '+$2', '15%', '-$244', 'بدترین'],
...
make_table(doc, ['معیار', 'Hermes واقعی', 'goldfree رپلی'], [
    ...
    ['سود نرمال‌شده', '+$44.21', '+$948'],
    ['حداکثر افت', '-$167.13', '-$59'],
    ['نرخ برد', '73.9%', '82%'],
    ['اجرا شده در سیستم', '23', '0'],
], widths=[36, 44, 78], center_cols=(1, 2))
para(doc, 'این مقایسه به نفع goldfree است اما منصفانه نیست: ۱۳ روز در برابر '
...
make_table(doc, ['تاریخ', 'رویداد', 'درس'], [
    ...
    ['۴ سپتامبر', 'تصحیح «۳۴ ترید» به «۲۳ پوزیشن»',
...
make_table(doc, ['مسیر', 'محتوا'], [
    ['/home/ai/hermes-trading', 'مخزن اصلی — ۴۰۳ فایل پایتون'],
    ...
    ['/home/ai/projects/forwarder-telegram-dockerized',
     'فورواردر — config با ۷ کانال مبدأ'],
    ['192.168.10.51:5050', 'بریج MT5 با Bearer auth'],
], widths=[52, 106])
