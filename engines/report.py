...
def render_execution_brief(plan: dict, execution: dict) -> str:
    command = execution.get("command") or execution.get("would_execute") or {}
    if isinstance(command, dict):
        side = _side_fa(command.get("side"))
        symbol = _symbol_fa(command.get("symbol") or plan.get("symbol"))
        volume = command.get("lot", "?")
        entry = command.get("entry") or command.get("entry_price") or "?"
        sl = command.get("sl", "?")
        tp = command.get("tp", "?")
    else:
