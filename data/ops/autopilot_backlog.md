...
- [ ] b41 Monkeypatch-the-wrong-binding audit (reusable procedure from b39's side finding): `from X import f` copies the function into the importer's namespace, so patching `X.f` silently changes NOTHING for that caller — test_market_closed_blocks_signal_order had been vacuous since the day it was written (the fake never reached auto_executor's binding; the REAL clock decided the outcome, and the test only failed once the market flipped). Audit every `patch.object(mod, 'attr')` / `mod.attr = fake` in tests/ against the production module's OWN from-import bindings (grep `from .* import` in the module under test), fix each to patch the binding the caller actually resolves, and add a tripwire test that fails if a test patches a name the production caller imported directly (same subprocess-freshness trick as test_b39_import_time_paths).
