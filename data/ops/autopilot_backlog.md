...
- [ ] b35 Runtime fallback time_exit uses the BROKER epoch as if it were UTC (found by b34): hermes_runtime._epoch_to_iso(getattr(p,'time')) feeds legacy_guards a broker-server-clock timestamp (~UTC+3), so on the fallback path a position looks 3h YOUNGER → the 36h time_exit fires ~3h LATE. The daemon fixed this properly (b32 broker_utc_offset_sec, NTP-style min-delay estimate from a live tick stream) but that machinery is overkill for a 15-min cycle. Cheapest safe option: reuse the daemon's calibration — it could persist its last accepted offset (e.g. in watchdog_state or a small data file) and the runtime reads it (0.0 = trust nothing = detection-time fallback, early-not-late). Low severity (fallback path only, errs late not early), but it is the same class of clock bug b32 killed.
