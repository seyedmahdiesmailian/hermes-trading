...
- [ ] b50 verify_head.sh must run the FULL suite on the clean checkout, not just the 2 import tests (found while harvesting b45 this run): commit 920ed0d shipped tests/test_b45_double_sell.py + probes but staged WITHOUT the two source fixes (selective staging — the b42 bug class' new variant: `git add -A` was skipped, and the b47 harvester missed it too because the dirty files were only ~25 min old, under the 60-min threshold). HEAD was genuinely BROKEN — 3 tests RED in a git-archive checkout — yet verify_head.sh would have said OK, because it only runs test_b42_tracked_imports + test_b44_clean_checkout (import integrity, not behaviour). Fix: after the import checks, run the whole `python3 -m unittest discover -s tests` inside the archived tree (suite is ~46s, fits the existing timeout; keep the import checks first so a ModuleNotFoundError still reports its specific shape), stamp the verdict as today. Never blocks, never touches trading paths.
