...
## Active
- [ ] b32 news_lock is ALIVE but only on the fallback path (found by b31): position management in production runs in position_daemon.py (watchdog heartbeat <60s → hermes_runtime's management block skips), and the daemon calls evaluate_trade_management directly — it never imports or evaluates legacy_guards, so the now-fixed news_lock still cannot protect a live position. Wire evaluate_news_lock (+ time_exit) into the daemon's 5s loop the same way hermes_runtime merges them (priority 1/2 ahead of the legacy chain), reusing the same runtime_state['management'] commit-on-broker-acceptance discipline from b7b/b10b. CAUTION: this changes live EXIT behaviour on a real demo account — do it behind a test that replays a blackout window against a fake bridge, and keep the tighten-only guard (already proven by test_never_loosens_stop).
