...
- [ ] b38 Verify b35 calibration handoff LIVE after market open (Monday ~23:00 UTC Sunday): data/xau_plan/broker_clock.json should appear within ~15s of the first fresh tick with offset_sec ≈ 10800 (CapitalXtend UTC+3); until then the daemon correctly publishes NOTHING (39h-old weekend tick = rejected, by design). If absent 1h after open, check position_daemon.log + _guard_cache['offsets'] path. Then confirm the next runtime fallback cycle reads it (log line or brief).
      **TIME-GATED — do NOT pick this item until the market is open (first fresh tick); pick the next todo instead.** Verified still absent at 2026-08-30 16:43 UTC (weekend, correct by design).
- [ ] b39 Import-time path binding: the class of bug b37 surfaced twice (found by b37): `LOG_FILE = _paths.logs_dir()/...` in hermes_master bound at IMPORT time, but `hermetic.use_temp_data_root()` flips HERMES_DATA_ROOT *after* import — so any test importing that module writes into PRODUCTION logs (the file's own comment claimed the opposite). hermes_master is fixed (call-time `_log_file()`/`_report_file()`, pinned by test_same_degradation_is_paged_once_per_cooldown which reads `_log_file()`); still open: notifier/telegram.py:10 `LOG_DIR` (currently harmless — every test patches send_ops/send_telegram, verified logs/telegram_messages.log untouched by the suite) and hermes_runtime.py:34 `BASE_DIR`/`DATA_DIR`/`PLAN_DIR` legacy aliases (live code uses `_plan_dir()`). Fix by converting to call-time accessors, then add a repo-wide test that imports every production module under a temp HERMES_DATA_ROOT and asserts no module-level constant still resolves under the ORIGINAL data root.
