...
working tree and the HEAD tree, plus fails on any untracked .py in the repo.
...
- [ ] b44 Post-commit HEAD verification (reusable procedure from b42's second occurrence): the b42 tripwire runs BEFORE the commit, so it can never see the damage the commit itself does — `git add -A` followed by tests, then `git commit`, leaves the NEW HEAD unverified (88cac1e and the untracked-b42-test-file both passed a pre-commit suite). Add scripts/verify_head.sh (or a git post-commit hook that logs, never blocks): after committing, re-run `python3 -m unittest tests.test_b42_tracked_imports` against the fresh HEAD and append the verdict to logs/; wire it as step 4b of the autopilot procedure in this file's header. Small, read-only, no live-system effect.
