...
- [ ] b42 Clean-checkout import integrity (found while verifying b40 on 2026-08-31): HEAD's hermes_master.py imported engines.guard_status, but the previous autopilot committed with `git commit -am` — which silently skips NEW untracked files — so a fresh clone/checkout of HEAD crashed with ModuleNotFoundError and every cron master tick would have died. Add tests/test_b42_tracked_imports.py: for every git-tracked top-level production module, AST-scan its `import X` / `from X import` for first-party modules and assert the resolved file is ALSO git-tracked (git ls-files, no subprocess archive needed); prove it bites by asserting the actual broken commit 88cac1e tree is flagged (git show 88cac1e:hermes_master.py through the same analyzer, like b41 does for the pre-b39 test). Also switch the autopilot commit step in this file's rules to explicit `git add -A` (already the stated rule — the tripwire makes violating it loud instead of fatal).
