...
- [ ] b37 The fallback path's guard merge is still `except Exception: pass` (found by b35): hermes_runtime.cycle wraps the news_lock/time_exit merge in a bare pass, so a broken plan shape or calendar error silently DROPS both safety guards from the fallback manager — and unlike position_daemon (which logs 'guard eval error'), the runtime does not even log it. b30 fail-closed the ENTRY gates; this is the same hole class on the management side. Minimum fix: log + put the error in the brief/return payload so the master report surfaces 'guards skipped'; decide deliberately whether a guard-eval error should also block new entries while a position is open.
