...
**Post-commit verification (b44, step 4b):** immediately AFTER the commit,
run `bash scripts/verify_head.sh`. It re-checks the FRESH HEAD (git archive
into a temp dir — no .env, no untracked files) with
tests.test_b42_tracked_imports + tests.test_b44_clean_checkout, appends the
verdict to logs/verify_head.log, and pages ops via Telegram if HEAD is broken.
It logs and alerts, never blocks. The pre-commit suite can only see the
working tree; only this step sees what the commit actually shipped.
...
- [ ] b49 Fail-safe scripts must not be indistinguishable from no-op scripts (reusable procedure from b48): autopilot_harvest.py, autopilot_digest's guard blocks and dirty_work readers all use `except Exception: pass`/print-nothing postures — correct for never blocking the run, but it means BROKEN and CLEAN produce byte-identical output (b48's behavioural test only caught it because a shadow package raised). Add a cheap self-verification seam: any print-only helper with a swallow-all main() should accept `--self-check` (or HERMES_SELFCHECK=1) that re-raises instead of swallowing and exits non-zero on any error, and a test that runs EVERY such script with --self-check against a fixture where it has output — so "printed nothing because broken" can never masquerade as "printed nothing because clean". Small, test+script-only, never touches trading paths.
- [ ] b45 git_sync pushes UNVERIFIED HEAD (found while wiring b44): scripts/git_sync.sh runs on cron every 15 min and pushes whatever HEAD is — including a HEAD that verify_head.sh would call BROKEN (the 43c5f52 shape: new module/test left untracked). The ops chat then gets a Telegram alert saying HEAD may die while GitHub already has the broken commit, and any future re-clone/restore-from-remote inherits it. Fix is small and read-only: have verify_head.sh stamp the verified sha into data/ops/head_verified.json ({sha, verdict, at}); git_sync.sh skips the push (logs it) when HEAD != last VERIFIED-OK sha AND the stamp is younger than ~1h (age guard so a dead verifier can never freeze pushes forever — fail-open on staleness, loud in the log). Never touches trading paths.
