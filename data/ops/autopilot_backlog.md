...
- [ ] b48 Test-seam env vars must redirect STATE, never CODE (reusable procedure from b47): the b47 first draft inserted HERMES_REPO_ROOT into sys.path, so a test pointing it at a throwaway repo made `from engines import dirty_work` die — and the script's own fail-safe swallowed the error, so the feature silently printed nothing (a test went RED with an empty string, not a crash). Any script with a HERMES_*_ROOT seam must import code from the fixed real repo (CODE_ROOT) and pass the env-var path ONLY as data to the scanner. Add an AST tripwire in tests/: every scripts/*.py that reads a HERMES_*_ROOT env var must not use that variable in a sys.path.insert argument; plus audit the existing seam users (autopilot_digest.py, autopilot_harvest.py) for the pattern. Small, test-only, never touches trading paths.
